...
export function popupParameters() {
  ...
  parameters += `,left=${(window.screen.width - 800) / 2},top=${
    (window.screen.height - 650) / 2
  }`;
  return parameters;
}

export function useIntuitAuthRedirect(onSuccess) {
    ...
    if (data && data.intuitAuth) {
      ...
      const pollOAuth = setInterval(async () => {
        try {
          if (
            !connectPopup ||
            (!connectPopup.document && !connectPopup.document.URL)
          )
            return;
          ...
          if (query.code) {
            ...
            connectPopup.close();
            throw new Error(
              isArray(query.error) ? query.error.join('') : query.error,
            );
          }
        } catch (e) {
          //console.log(e);
