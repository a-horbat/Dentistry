    ...
    <Box className={classes.progressBox} style={style || {}}>
          ...
          .map((_, index) => {
            return (
              <Box
                ...
                style={{ left: `calc(${33.333 * index}% - ${7 * index}px)` }}
              />
            );
          })}
        <Box
          className={classes.circleCurrent}
          style={{ left: `calc(${33.333 * step}% - ${7 * step}px)` }}
        />
        {Array(3 - step)
          .fill(null)
          .map((_, index) => {
      ...
      <Box className={classes.labelsContainer}>
        {Object.keys(stepMap).map((key, index) => {
          ...
          const leftStyle =
            index === 1
              ? { left: `calc(${33.333 * index}% - ${43 * index}px)` }
              : index === 2
              ? { left: `calc(${33.333 * index}% - ${26 * index}px)` }
              : { left: `calc(${33.333 * index}% - ${31 * index}px)` };
          return (
            <Typography
